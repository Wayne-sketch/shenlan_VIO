...
//输入body坐标系下的欧拉角，输入的欧拉角意义是按照x(roll)，y(pitch)，z(yaw)轴的顺序旋转到inertial坐标系下
//输出为body to inertial的旋转矩阵
Eigen::Matrix3d euler2Rotation(Eigen::Vector3d eulerAngles)
{
    // 取出欧拉角的roll pitch yaw角度
    ...
    double cy = cos(yaw);
    double sy = sin(yaw);
    // 将body坐标系下给定的欧拉角转换成body to inertial的旋转矩阵 PPT45页公式
    Eigen::Matrix3d RIb;
...
//问题很大，想不明白欧拉角速度和body角速度转换的具体问题设定是什么，没想明白要做的是什么事情，有什么已知条件？？？？？？？？？？？？？？？？？？？
//输入为inertial坐标系下的欧拉角，输入的欧拉角的意义是按照z(yaw)，y(pitch)，x(row)轴的顺序旋转到body坐标系下
//输出为inertial frame下的欧拉角速度转到body坐标系下的矩阵，对应PPT公式37
...
void IMU::addIMUnoise(MotionData &data)
{
    ...
    // 生成陀螺仪仿真数据，包括高斯白噪声和bias随机游走，有疑问？？？？？
...
MotionData IMU::MotionModel(double t)
{
    ...
    */
    ...
    // body frame to world frame 的欧拉角
    ...
    // body frame to world frame 的旋转矩阵
...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    // 将src中的IMU数据数据存入MotionData容器imudata中
