...
namespace rootBA {
    ...
    bool LandmarkBlock<Scalar>::PrecomputeResidual(void) {
        ...
        for (auto &observe : this->observes) {
            ...
            //观测到的归一化平面的坐标，追踪一下哪些步骤改变了norm的值！！！！！！！！！！！！！！！！！
            ...
            if (std::isnan(invDep) || std::isinf(invDep)) {  // 判断这个点是否正常
                ...
                //norm是通过相机位姿，landmark位置计算出来的归一化平面坐标，是待优化的状态量
                //measure是observe里存储的归一化平面坐标，带噪声的
    ...
    bool LandmarkBlock<Scalar>::ComputeResidual(void) {
        ...
        for (auto &residual : this->residuals) {
            //给camera id 得到相应的storage矩阵的索引值
            size_t idx = (*this->cameraID_idx.find(residual.first)).second;
            //这里把预计算的residual放入storage矩阵里，每个相机占storage矩阵的两行，residual放在storage矩阵的最后一列
    ...
    bool LandmarkBlock<Scalar>::ComputeJacobian(void) {
        ...
        for (auto &observe : this->observes) {
            ...
            // 利用待优化的变量计算出的观测的归一化平面坐标 norm = [ux, uy, 1].T
            Vector3<Scalar> p_b = R_bw * (p_w - t_wb);
            Vector3<Scalar> p_c = R_cb * (p_b - t_bc);
            Scalar dep = p_c.z();

            // 计算雅可比矩阵 d(归一化平面位置) / d(相机坐标系位置) 存在reduce中
...
//本部分自己最初的推导有问题，参考PDF推导！！！！！！！！！！！！！！！！！！！！！！！！！！！！1
    ...
    bool LandmarkBlock<Scalar>::PerformQR(void) {
        ...
        Eigen::HouseholderQR<MatrixX<Scalar>> Jl_QR(Jl);
        // Jl_QR.compute(Jl);
        // MatrixX<Scalar> R = Jl_QR.matrixQR().triangularView<Eigen::Upper>();
	    MatrixX<Scalar> Q = Jl_QR.householderQ();
        // // 对 storage 矩阵左乘 Q.T
    ...
    bool LandmarkBlock<Scalar>::RollBack(void) {
        ...
        this->storage = Q_lambda.transpose() * this->storage;
        // Fill in above this line
